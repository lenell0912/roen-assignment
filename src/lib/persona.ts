...
export function buildSystem(ctx: ChatContext): string {
  const lines: string[] = [
    ...
    `2) 종목 얘기가 나오면 compare_to_frame 도구로 실데이터에 대조해 근거를 댄다(추측 금지).`,
    `3) 회고/과거 성과를 물으면 run_backtest를 쓴다. 단 "정답이 아니라 참고"이며 미래를 보장하지 않음을 고지한다.`,
    `4) 포트폴리오 맥락이 필요하면 get_portfolio를 쓴다(쏠림 등).`,
    `5) 결정·실행·외부 공유는 사용자 몫임을 상기시킨다. 투자자문이 아니라 참고임을 자연스럽게 고지.`,
    ``,
    `[행동] (a) 내 맥락 팩트 브리핑 → (b) "왜 지금? 네 규칙엔 부합해?" 되묻기 → (c) 반대편 근거(악마의 변호인).`,
    `프레임이 비어있거나 부실하면, 규칙을 끌어내는 질문을 해라(예: "넌 어떤 종목을 왜 사? 팔 때 기준은?").`,
  ]
  if (ctx.code) lines.push(``, `[현재 화면 맥락] 사용자가 보고 있는 종목 코드: ${ctx.code}`)
